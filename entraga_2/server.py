...
def receive_message():
    ...
    while True:
        ...
        if initial_message.decode("ISO-8859-1").startswith('*$*') or initial_message.decode("ISO-8859-1").startswith('*#*'):
            messages_queue.put(('$#', initial_message, ip_client, 0))
            continue
        ...
        for address in clients_address:
            if address == ip_client:
                ...
                with open(file, encoding="ISO-8859-1") as txt:
                    ...
                    if packageIndex + 1 == packagesQuantity:
                        message_to_send = f'{message_to_send} {time_data}'
                        
                    
                    # message_to_send = f'{ip_client[0]}:{ip_client[1]}/~{name}: "{message}" {time_data}'.encode("ISO-8859-1")
                    message_to_send = message_to_send.encode("ISO-8859-1")

                    while messages_queue.qsize() > 0:
                        time_to_wait = 0.1
                        time.sleep(time_to_wait)

                    messages_queue.put((packagesQuantity, message_to_send, ip_client, packageIndex))

def broadcast():
    while True:
        while messages_queue.qsize() != 0:
            package_quantity_or_command, encoded_message, ip_client, packageIndex = messages_queue.get()
            ...
            encoded_message = f"{message} *PKT-{packageIndex}-{package_quantity_or_command}*".encode("ISO-8859-1")
